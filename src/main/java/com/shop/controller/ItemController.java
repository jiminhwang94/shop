...
public class ItemController {
    ...
    @GetMapping(value = {"/admin/items", "/admin/items/{page}"})        //URL에 페이지가 없는 경우와 있는 경우 2가지 매핑.
    public String itemManage(ItemSearchDto itemSearchDto, @PathVariable("page") Optional<Integer> page, Model model){
        //PageRequest.of 메소드를 통해 Pageable 객체 생성.
        // 첫 번째 파라미터 :조회할 페이지 번호, 두 번째 파라미터 : 한번에 가지고 올 데이터 수, URL경로에 페이지 번호가 있으면 해당 페이지를 조회하도록 세팅, 없으면 0페이지 조회
        ...
        Page<Item> items = itemService.getAdminItemPage(itemSearchDto,pageable);            //조회 조건과 페이징 정보를 파라미터로 넘겨서 Page<Item> 객체를 반환
                            ...
                            model.addAttribute("maxPage",5);        //페이지 번호의 최대 개수. 최대 5개의 페이지 번호만 보여준다.
                            return "item/itemMng";
    }
    ...
    public String itemDtl(@PathVariable("itemId") Long ItemId, Model model){
        ...
        } catch (EntityNotFoundException e){        // 상품 엔티티가 존재하지 않으면 에러메시지 담아서 상품 등록 페이지로 이동
